...
- `ont` follows `field-inventory.v1.json`;
...
- authored `ontology.ttl` uses RDF 1.1 named IRIs and literals only and contains no blank nodes;
- protocol-owned `shapes.ttl` may use SHACL blank-node property/list structures; those nodes are validation configuration, never source semantic facts, canonicalizer input, RDF round-trip payload, or semantic-digest input;
- no RDF-star;
- no lists/containers;
- no imports;
...
```json
{
  ...
  "guidance": {"examples":[],"anti_examples":[],"sections":[],"system4d":null},
