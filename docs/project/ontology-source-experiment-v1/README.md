---
summary: "Frozen v1 protocol bundle for the four-way ontology source architecture comparison."
read_when:
...
# Ontology source experiment protocol v1
...
Protocol revision v1 intentionally retains nested experimental source-format identifiers ending in `.v0` (`rocs-source-experiment.v0`, `rocs-semantic-object-experiment.v0`, and the bounded RDF v0 profile). Those identify unchanged candidate encodings; the surrounding execution protocol, accepted inputs, scoring, and evidence rules are v1.

## Frozen bundle

| Artifact | Purpose |
|---|---|
| `protocol-manifest.v1.json` | SHA-256 inventory and aggregate digest of every other frozen protocol artifact. |
| `field-inventory.v1.json` | Exhaustive classification and normalization of observed and synthetic fields. |
| `baseline-manifest.v1.json` | Exact Git tree, fixture paths, Git blobs, and SHA-256 source-byte digests. |
| `fixture-files.v1.json` and `fixtures/` | Exact frozen V1–V4 baseline source bytes and their SHA-256 manifest. |
| `accepted-inputs-manifest.v1.json` and `accepted-inputs/` | Forty byte-exact source packets: every golden case for every variant. |
| `task-fixtures.v1.json` | Exact T1–T6 prompts, starting overlays, branch bytes, digests, and expected golden cases. |
| `golden-cases.v1.json` | Exact accepted-case canonical fact streams and semantic digests. |
| `malformed-cases.v1.json` | Exact adversarial payloads, applicability, errors, and precedence. |
| `variant-contracts.v1.md` | Frozen V1–V4 syntax, topology, dependency, and maintained/generated boundaries. |
| `decision-rule.v1.json` | Mechanical correctness, complexity, dominance, tie, and no-winner rules. |
| `complexity-measurement.v1.json` | Exact median, LOC, contract-byte, dependency, error, and surface algorithms. |
| `evidence-manifest-contract.v1.json` | Exact retained-evidence and operator-status byte verification contract. |
| `actor-materials/` | Frozen actor/reviewer instructions, tools, validation help, warm-up, blind mapping, and normalization. |
| `runbook.v1.md` | Independent implementation, ergonomics, isolation, evidence, and cleanup protocol. |
...
2. Numbers are forbidden in protocol v1 semantic facts; booleans are JSON `true` or `false`; missing optional values are represented by absence of a fact, not `null`.
...
Allowed protocol v1 fact shapes are closed:
...
The protocol v1 semantic fact stream excludes guidance, System4D, layout, source provenance, owner authority, and current-head facts. Those remain available to source/debug and projection-loss reports where required.
...
The earlier phrase “label-only rename preserves semantic identity” is replaced by the precise requirement: a label rename preserves `term_id` but changes `semantic_digest` in protocol v1.
...
- all `system4d` content is non-normative guidance in protocol v1 and does not participate;
...
There are no `if selected` fields in protocol v1.
...
`golden-cases.v1.json` contains these exact cases:
...
`malformed-cases.v1.json` controls applicability. An implementation cannot choose `not_applicable`. Every applicable payload has:
...
When multiple errors exist, implementations must return the lexically first error code in this table's controlling numeric precedence from `malformed-cases.v1.json`; no parser-native accident decides the result.
...
For correctness-passing variants, calculate the exact complexity vector defined in `decision-rule.v1.json`:
...
Protocol v1 uses eight fresh stateless Pi sessions running `openai-codex/gpt-5.6-sol` under the same checked-in tool/instruction surface. Each actor performs the six selecting authoring tasks for all four variants under the Latin-square order in `runbook.v1.md`. The remaining projection and rejection tasks are correctness tests, not ergonomics samples.
