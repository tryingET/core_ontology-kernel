---
summary: "Frozen isolation, independent implementation, ergonomics, evidence, and cleanup runbook for ontology source experiment v1."
read_when:
  - "Executing or reviewing ontology source experiment v1."
type: procedure
status: proposed
---

# Experiment runbook v1
...
- baseline commit and `baseline-manifest.v1.json` verification;
...
Compute the baseline source-tree digest exactly as specified in `baseline-manifest.v1.json`. Capture the immutable baseline status and the operator dirty-checkout status as exact bytes from `git status --porcelain=v1 -z --untracked-files=all`, without decoding or sorting, and retain each byte stream plus SHA-256. Untracked files are impossible in the clean baseline; if present, preflight fails. The operator status bytes may be dirty but must compare byte-for-byte equal at closeout.

## Freeze order

1. Freeze and digest this protocol bundle.
2. Verify and materialize exact accepted inputs from `accepted-inputs-manifest.v1.json`, exact T1–T6 starts from `task-fixtures.v1.json`, and exact malformed payloads from the bundle.
...
Changing a protocol, fixture, oracle, expected error, dependency class, actor material, complexity rule, or decision rule after step 4 creates protocol v2; v1 execution stops.
...
Fixed per-case timeout: 10 seconds wall clock and 256 MiB additional resident-memory budget. A timeout returns `E_TIMEOUT` and fails because `E_TIMEOUT` is not an expected v1 error.
...
- exact actor materials from `actor-materials/manifest.v1.json`: `actor-system-instructions.txt`, `tool-allowlist.v1.json`, `validation-help.v1.json`, and `warmup.v1.json`;
- byte-exact task prompt and starting fixture from `task-fixtures.v1.json`;
...
Apply `decision-rule.v1.json` mechanically. Report exactly one:
