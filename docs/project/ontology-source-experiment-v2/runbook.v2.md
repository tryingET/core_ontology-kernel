---
summary: "Frozen isolation, independent implementation, ergonomics, evidence, and cleanup runbook for ontology source experiment v2."
read_when:
  - "Executing or reviewing ontology source experiment v2."
type: procedure
status: proposed
---

# Experiment runbook v2
...
- baseline commit and `baseline-manifest.v2.json` verification;
...
Compute the baseline source-tree digest exactly as specified in `baseline-manifest.v2.json`. Capture the immutable baseline status and the operator dirty-checkout status as exact bytes from `git status --porcelain=v1 -z --untracked-files=all`, without decoding or sorting, and retain each byte stream plus SHA-256. Untracked files are impossible in the clean baseline; if present, preflight fails. The operator status bytes may be dirty but must compare byte-for-byte equal at closeout.

## Freeze order

1. Freeze and digest this protocol bundle.
2. Verify and materialize exact accepted inputs from `accepted-inputs-manifest.v2.json`, exact T1–T6 starts from `task-fixtures.v2.json`, and exact malformed payloads from the bundle.
...
Changing a protocol, fixture, oracle, expected error, dependency class, actor material, complexity rule, or decision rule after step 4 creates protocol v3; v2 execution stops.
...
The 40 byte-exact accepted source packets under `accepted-inputs/`, their `accepted-inputs-manifest.v2.json`, and the exact expected streams in `golden-cases.v2.json` are controlling. Source reorder and Unicode-equivalence inputs must prove equal digests where declared. Label, definition, edge, and lifecycle cases must prove the exact identity relationships declared by the oracle. No executor creates or edits an accepted input.
...
Fixed per-case timeout: 10 seconds wall clock and 256 MiB additional resident-memory budget. A timeout returns `E_TIMEOUT` and fails because `E_TIMEOUT` is not an expected v2 error.
...
Actors may use only the commands and shell grammar in `actor-materials/tool-allowlist.v2.json`. Warm-up uses only `./validate-toy`. Scored validation and V4 authoring use only the A/B-agreement wrapper in `actor-materials/experiment-wrapper-contract.v2.json`; implementation disagreement stops the cohort as a correctness failure and is never scored as actor error. Actors may not inspect another variant during a task.

## Actor cohort

- eight fresh stateless Pi sessions;
- exact provider/model: `openai-codex/gpt-5.6-sol`;
- exact actor materials from `actor-materials/manifest.v2.json`: `actor-system-instructions.txt`, `tool-allowlist.v2.json`, `validation-help.v2.json`, and `warmup.v2.json`;
- one byte-identical mode-`0755` `./experiment` wrapper per scored root, implemented exactly by `actor-materials/experiment-wrapper-contract.v2.json` after A/B lock;
- byte-exact task prompt and starting fixture from `task-fixtures.v2.json`;
...
Two additional fresh stateless reviewer sessions use the same exact model/provider and exact `actor-materials/reviewer-system-instructions.txt`; they do not author tasks. The comparator applies `actor-materials/reviewer-presentation.v2.json` exactly, including its fixed blind mapping, key order, normalization, omissions, packet fields, and output schema. Reviewers receive only the normalized packet and cannot read the mapping or protocol repository.
...
Reviewer disagreement and all scores are retained as descriptive evidence only. They never alter machine correctness, `authoring_task_errors`, any complexity dimension, eligibility, dominance, or final disposition. No subjective adjudicator selects a winner.

## Complexity evidence

After implementations lock, a fourth read-only measurement process applies `complexity-measurement.v2.json` exactly. It emits every required raw receipt, recomputes frozen contract bytes, rejects unclassified paths/dependencies, and produces the integer/rational comparison values and Pareto matrix. Missing or invalid values produce `no_winner_insufficient_evidence`; they are never imputed.
...
Apply `evidence-manifest-contract.v2.json` exactly and create `evidence-manifest.v2.json` last. It freezes file types, path normalization/order, entry bytes, digest preimage, self-exclusion, independent verification, operator/baseline status byte capture, and cleanup gate. Only after independent verification may disposable variant and implementation copies be deleted. Never delete the retained evidence root automatically.
...
- operator dirty-checkout porcelain-v1-z byte stream and SHA-256 equal the captured preflight bytes exactly;
- all experiment outputs are outside both source checkouts;
- retained evidence manifest verifies.

## Final disposition

Apply `decision-rule.v2.json` mechanically. Report exactly one:
