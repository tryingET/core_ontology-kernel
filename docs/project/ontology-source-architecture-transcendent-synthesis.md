...
| Verified v2 observation | What survives | What does not follow |
|---|---|---|
...
1. **Completed — source-identity purpose gate:** independent review passed with `not_needed`; Stage B was not entered, and normalized-digest implementation is closed for this scoped disposition.
2. **Dormant — source-identity reopening:** only a new consumer satisfying the reopening rule may restart Stage A.
3. **Conditional — capability-specific test:** only an owner-named capability, such as a guarded visual-proposal operation, may justify its own smallest protocol. It cannot inherit source-format or authority conclusions.
4. **RFC/ADR:** blocked until separate, protocol-valid evidence demonstrates a source-format winner under an independently reviewed decision rule.
...
- Projection and release contracts cannot inherit experimental digests.
